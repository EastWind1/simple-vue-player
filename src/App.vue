<template>
  <div class="d-flex justify-content-center">
    <div>
      ...
      <!-- 播放器 -->
      <div>
        <audio ref="player" controls :src="currentPlay.src" @ended="playEnd()"></audio>
      </div>
      <br/>
       <!-- 文件导入 -->
      <div>
      ...
      <div>
        <div class="form-check">
          <input
            ...
            id="listCycle"
          />
          <label class="form-check-label" for="listCycle">
            列表循环
          </label>
        </div>
...
export default class App extends Vue {
  ...
  fileUpload(): void {
    ...
    if (files && files.length) {
      const file = files[0];
      const existIndex = this.playList.findIndex((item) => item.name == file.name);
      let playIndex = 0;
      if (existIndex != -1) {
   ...
   * 绘制
   */
  paintxWave(data: Uint8Array): void {
